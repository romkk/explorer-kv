...
void usage() {
  fprintf(stderr, "Usage:\n\tpre_txdb -c \"tparser.bootstrap.conf\" -l \"pre_txdb.log\"\n");
}
...
int main(int argc, char **argv) {
  ...
  // write pid to file
  writePid2FileOrExit("pre_txdb.pid");
  ...
  try {
    gPreTxDB = new PreTxDB();
    gPreTxDB->run();
    delete gPreTxDB;
    gPreTxDB = nullptr;
  } catch (std::exception & e) {
    LOG_FATAL("pre_txdb exception: %s", e.what());
    return 1;
  }
  return 0;
}
