...
NotifyEventsProducer *gProducer = nullptr;
...
void usage() {
  string u = "Usage:\n";
  u += "\tnotifyproducer -c \"notifyproducer.conf\" -l \"notifyproducer.log\"\n";
...
int main(int argc, char **argv) {
  ...
  // write pid to file
  const string pidFile = "notifyproducer.pid";
  ...
  try {
    gProducer = new NotifyEventsProducer();
    gProducer->init();
    gProducer->run();
    ...
    LOG_FATAL("notifyproducer exception: %s", e.what());
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
}
