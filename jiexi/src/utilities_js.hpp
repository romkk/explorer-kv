...
class Node{
  public:
    ...
    int64_t get_int64_or_throw(const char * key) {
      Node node = (*this)[key];
      if (node.type_ != JS::type::Int) {
        THROW_EXCEPTION_EX(EINVAL, "json get_int64_or_throw: %s", key);
      } else {
        errno = 0;
        int64_t ret = node.int64();
        if (errno == 0) {
          return ret;
        } else {
    ...
    uint64_t get_uint64_hex_or_throw(const char * key) {
      Node node = (*this)[key];
      if (node.type_ != JS::type::Str) {
        THROW_EXCEPTION_EX(EINVAL, "json get_uint64_hex_or_throw: %s", key);
      } else {
        errno = 0;
        int64_t ret = node.uint64_hex();
        if (errno == 0) {
          return ret;
        } else {
          throw std::logic_error(strerror(errno));
    ...
    std::string get_str_or_throw(const char * key) {
      Node node = (*this)[key];
      if (node.type_ != JS::type::Str) {
        THROW_EXCEPTION_EX(EINVAL, "json get_str_or_throw: %s", key);
      } else {
        return node.str();
      }
    }

    std::vector<Node> & get_array_or_throw(const char * key) {
      Node node = (*this)[key];
      if (node.type_ != JS::type::Array) {
        THROW_EXCEPTION_EX(EINVAL, "json get_array_or_throw: %s", key);
      } else {
        return node.array();
      }
    }
